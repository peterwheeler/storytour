...
L.TileLayer.GeoZoomify = L.TileLayer.extend({
	options: {
	...
	onAdd: function (map) {
		...
		var mapSize = map.getSize(),
			...
			center = (L.latLng(imageSize.x / 2, imageSize.y / 2), zoom);
			// center = (L.latLng(41.796913, 12.237609), zoom);
		//map.setView(center, zoom, true)
     		// console.log(map.getSize(), map.getCenter(), map.latLngToLayerPoint(L.latLng(41.796913, 12.237609)));
     		console.log(_getTileGroup());
	},
	
	_getZoomifyBounds: function(map) {
	...
	transformToReal: function(){
		var transform = L.transformation(2, 5, -1, 10);
		console.log(trans.transform(L.point(center.lat, center.lng)));
	},
...
L.tileLayer.geoZoomify = function (url, options) {
	return new L.TileLayer.GeoZoomify(url, options);
};